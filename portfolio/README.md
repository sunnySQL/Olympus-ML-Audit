# Olympus Portfolio Export

Generated: `2026-05-19T04:18:26.714173+00:00`

## Verdict

- Deployment: `REJECT_FOR_TRADING`
- Project: `PORTFOLIO_READY_MODEL_AUDIT_PLATFORM`
- Target: `target_intraday_next_direction`
- Feature set: `v10`

## Main Artifacts

- `olympus_case_study.md`
- `olympus_model_audit_report.md`
- `olympus_model_audit_report.html`

## Result Tables

- `artifacts/model_audit_summary.csv`
- `artifacts/model_audit_summary.json`
- `artifacts/model_target_benchmark.csv`
- `artifacts/backtest_comparison.csv`
- `artifacts/ranking_alpha_summary.csv`
- `artifacts/rank_feature_sweep.csv`
- `artifacts/walk_forward.csv`
- `artifacts/walk_forward_next3d.csv`

## Screenshots

- `screenshots/auditlog.png` - Audit checklist
- `screenshots/backtest.png` - Backtest vs baselines
- `screenshots/chart.png` - Ticker chart and signal context
- `screenshots/landingpage.png` - Dashboard landing page
- `screenshots/modelhealth.png` - Model health

